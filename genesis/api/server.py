...
from genesis.services.assistant import AssistantError
...
from genesis.services.world import WorldError
...
class NeoGenApiHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        try:
            ...
            if path == "/api/v1/assistant":
                self._send(HTTPStatus.OK, self.kernel.assistant.snapshot(user.id, context=self.kernel.assistant_context(user.id)))
                return
            if path == "/api/v1/assistant/memory":
                self._send(HTTPStatus.OK, {"items": self.kernel.assistant.memories(user.id, limit=int(query.get("limit", [50])[0]))})
                return
            if path == "/api/v1/assistant/plans":
                self._send(HTTPStatus.OK, {"items": self.kernel.assistant.plans(user.id, limit=int(query.get("limit", [25])[0]))})
                return
            if path == "/api/v1/assistant/audit":
                self._send(HTTPStatus.OK, {"items": self.kernel.assistant.audit(user.id, limit=int(query.get("limit", [100])[0]))})
                return
            if path == "/api/v1/assistant/settings":
                self._send(HTTPStatus.OK, self.kernel.assistant.settings(user.id))
                return
            if path == "/api/v1/governance":
                self._send(HTTPStatus.OK, self.kernel.governance_snapshot())
                return
            if path == "/api/v1/world/regions":
                self._send(HTTPStatus.OK, {"items": self.kernel.world.regions()})
                return
            if path == "/api/v1/world/profile":
                self._send(HTTPStatus.OK, self.kernel.world.profile(user.id))
                return
            if path == "/api/v1/world/expeditions":
                self._send(HTTPStatus.OK, {"items": self.kernel.world.expeditions(user.id)})
                return
            if path == "/api/v1/avatar":
                self._send(HTTPStatus.OK, self.kernel.game.get_or_create_avatar(user.id))
                return
            if path == "/api/v1/inventory":
                self._send(HTTPStatus.OK, {"items": self.kernel.game.inventory(user.id)})
                return
            if path == "/api/v1/wallet":
                self._send(HTTPStatus.OK, self.kernel.game.wallet(user.id))
                return
            if path == "/api/v1/wallet/transactions":
                self._send(HTTPStatus.OK, {"items": self.kernel.game.transactions(user.id)})
                return
            if path == "/api/v1/marketplace":
                self._send(HTTPStatus.OK, {"items": self.kernel.game.listings()})
                return
            ...
            if path == "/api/v1/workspace/list":
                self._send(HTTPStatus.OK, {"items": self.kernel.workspace.list(
                    query.get("path", [""])[0],
            ...
            if path == "/api/v1/checkpoints":
                self._send(HTTPStatus.OK, {"items": [record.value for record in self.kernel.storage.list(self.kernel.checkpoints.namespace)]})
                return
        ...
        except (AssistantError, ConversationError, WorkspaceError, GameError, WorldError, ValueError) as exc:
    ...
    def do_POST(self) -> None:  # noqa: N802
        try:
            ...
            if path == "/api/v1/auth/login":
                ...
                self._bootstrap_user(session.user_id)
            ...
            user = self._authenticated_user()
            if path == "/api/v1/assistant/memory":
                self._send(HTTPStatus.CREATED, self.kernel.assistant.remember(
                    user.id,
                    title=self._required(payload, "title"),
                    content=self._required(payload, "content"),
                    category=str(payload.get("category", "user")),
                    importance=float(payload.get("importance", 0.6)),
                ))
                return
            if path == "/api/v1/assistant/plans":
                self._send(HTTPStatus.CREATED, self.kernel.assistant.create_plan(user.id, goal=self._required(payload, "goal")))
                return
            plan_id = self._assistant_plan_path(path)
            if plan_id:
                self._send(HTTPStatus.OK, self.kernel.assistant.set_plan_status(user.id, plan_id, self._required(payload, "status")))
                return
            if path == "/api/v1/assistant/settings":
                self._send(HTTPStatus.OK, self.kernel.assistant.update_settings(user.id, payload))
                return
            if path == "/api/v1/world/expeditions":
                avatar = self.kernel.game.get_or_create_avatar(user.id)
                outcome = self.kernel.world.begin_expedition(
                    user.id,
                    self._required(payload, "region_id"),
                    avatar_level=int(getattr(avatar, "level", 1)),
                )
                reward = int(outcome["result"].get("reward_acoin", 0))
                if reward > 0:
                    outcome["wallet"] = self.kernel.game.credit(user.id, reward, reason=f"Expedition: {outcome['result']['region_name']}")
                self._send(HTTPStatus.CREATED, outcome)
                return
            if path == "/api/v1/avatar":
                self._send(HTTPStatus.OK, self.kernel.game.update_avatar(
                    user.id,
                    name=str(payload["name"]) if payload.get("name") is not None else None,
                    appearance=dict(payload["appearance"]) if payload.get("appearance") is not None else None,
                ))
                return
            if path == "/api/v1/inventory/grant":
                self._require_admin(user)
                self._send(HTTPStatus.CREATED, self.kernel.game.grant_item(
                    str(payload.get("user_id") or user.id),
                    item_type=self._required(payload, "item_type"),
                    name=self._required(payload, "name"),
                    rarity=int(payload.get("rarity", 1)),
                    metadata=dict(payload.get("metadata", {})),
                ))
                return
            if path == "/api/v1/wallet/credit":
                self._require_admin(user)
                self._send(HTTPStatus.OK, self.kernel.game.credit(
                    str(payload.get("user_id") or user.id),
                    int(payload.get("amount", 0)),
                    reason=str(payload.get("reason", "reward")),
                ))
                return
            if path == "/api/v1/marketplace/list":
                self._send(HTTPStatus.CREATED, self.kernel.game.create_listing(
                    user.id,
                    name=self._required(payload, "name"),
                ...
                return
            if path == "/api/v1/marketplace/purchase":
                self._send(HTTPStatus.OK, self.kernel.game.purchase(user.id, self._required(payload, "listing_id")))
                return
            if path == "/api/v1/conversations":
                self._send(HTTPStatus.CREATED, self.kernel.conversations.create(user_id=user.id, title=str(payload.get("title", "New conversation"))))
                return
            ...
            if path == "/api/v1/workspace/delete":
                self._send(HTTPStatus.OK, self.kernel.workspace.delete(self._required(payload, "path"), recursive=bool(payload.get("recursive", False))))
                return
            ...
            if path == "/api/v1/permissions/grant":
                self._require_admin(user)
                try:
                    scope = PermissionScope(self._required(payload, "scope"))
                except ValueError as exc:
                    raise ApiError(HTTPStatus.BAD_REQUEST, f"Unknown permission scope: {payload.get('scope')}") from exc
            ...
            self._send(HTTPStatus.UNAUTHORIZED, {"error": str(exc)})
        except (AssistantError, ConversationError, WorkspaceError, TerminalError, GameError, WorldError, ValueError) as exc:
            self._send(HTTPStatus.BAD_REQUEST, {"error": str(exc)})
        except Exception as exc:
    ...
    def _bootstrap_user(self, user_id: str) -> None:
        self._ensure_chat_permissions(user_id)
        self.kernel.game.get_or_create_avatar(user_id)
        self.kernel.world.profile(user_id)
        self.kernel.assistant.settings(user_id)
    ...
    @staticmethod
    def _assistant_plan_path(path: str) -> str | None:
        prefix = "/api/v1/assistant/plans/"
        suffix = "/status"
        if not path.startswith(prefix) or not path.endswith(suffix):
            return None
        return unquote(path[len(prefix):-len(suffix)].strip("/")) or None
...
def create_server(
    *,
    ...
    kernel = NeoGenKernel.build(storage_path=storage_path, workspace_path=workspace_path, enable_puter=enable_puter)
